...
imageY = 160
imageZ = 3
totalPixels = imageX*imageY*imageZ

class Qnetwork():
    def __init__(self,h_size):
        #The network recieves a frame from the game, flattened into an array.
        #It then resizes it and processes it through four convolutional layers.
        self.scalarInput =  tf.placeholder(shape=[None,totalPixels],dtype=tf.float32)
        self.imageIn = tf.reshape(self.scalarInput,shape=[-1,imageX,imageY,imageZ])
        self.conv1 = slim.conv2d( \
            inputs=self.imageIn,num_outputs=32,kernel_size=[14,10],stride=[4,4],padding='VALID', biases_initializer=None)
...
def processState(states):
    return np.reshape(states,[totalPixels])
